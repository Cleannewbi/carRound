...
SELECT COM_NAME FROM RC_INFO WHERE SEQ=(SELECT COM_NUM FROM RC_RENT) AND COM_NAME='기아오토큐'
이러면 RC_RENT에 있는 회사들 이름이 나옴
이름별로 몇개인지 걸러야함
SELECT ID, NAME, PHONE, CARD FROM RC_MEMBER  WHERE AUTH=1 OR AUTH=2 ORDER BY AUTH ASC 
SELECT * FROM RC_MEMBER